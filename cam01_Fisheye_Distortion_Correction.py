...
class DualImagePostProcessor:
    def __init__(self):
        ...
        self.crop_params = {
            'cam0': {'width': 2070, 'start_x': 1260, 'height': 2592, 'start_y': 0},  # Left camera
            'cam1': {'width': 2050, 'start_x': 1400, 'height': 2592, 'start_y': 0}   # Right camera
        }
        ...
        # Padding parameters for correction functions
        self.padding = 100  # Padding for distortion correction
        
       
        # crop parameters for after correction
        self.final_crop_params = {
            'cam0': {'width': 2050, 'start_x': 170, 'height': 2700, 'start_y': 70},  # Left camera
            'cam1': {'width': 2020, 'start_x': 200, 'height': 2700, 'start_y': 70}   # Right camera
        }
    ...
    def crop_image(self, image, cam_name, params):
        """Crop image according to camera-specific parameters"""
        if not self.apply_cropping or cam_name not in params:
            return image
            
        start_x = params[cam_name]['start_x']
        start_y = params[cam_name]['start_y']
        width = params[cam_name]['width']
        height = params[cam_name]['height']
    ...
    def apply_distortion_correction(self, image, cam_name):
        ...
        try:
            ...
            # Add padding to prevent cutoff during correction
            ...
            if image.ndim == 2:
                # Grayscale image
                padded_image = np.zeros((height + 2*padding, width + 2*padding), dtype=image.dtype)
                padded_image[padding:padding+height, padding:padding+width] = image
            else:
                # Multi-channel image
                padded_image = np.zeros((height + 2*padding, width + 2*padding, image.shape[2]), dtype=image.dtype)
                padded_image[padding:padding+height, padding:padding+width, :] = image
            
            # Adjust center coordinates for padded image
            padded_xcenter = xcenter + padding
            padded_ycenter = ycenter + padding
            
            # Convert to float for processing
            if padded_image.dtype != np.float64:
                image_float = padded_image.astype(np.float64)
            else:
                image_float = padded_image.copy()
            
            if image_float.ndim == 2:
                # Grayscale image
                corrected = post.unwarp_image_backward(image_float, padded_xcenter, padded_ycenter, coeffs, order=2, mode='constant')
            else:
                # Multi-channel image
                corrected = np.zeros_like(image_float)
                for c in range(image_float.shape[2]):
                    corrected[:, :, c] = post.unwarp_image_backward(image_float[:, :, c], padded_xcenter, padded_ycenter, coeffs, order=2, mode='constant')
            ...
            print(f"   Center: ({xcenter:.1f}, {ycenter:.1f}) -> ({padded_xcenter:.1f}, {padded_ycenter:.1f})")
            print(f"   Added padding: {padding} pixels")
            print(f"   Image size: {image.shape} -> {corrected.shape}")
    ...
    def apply_perspective_correction(self, image, cam_name):
        ...
        try:
            ...
            # Add padding to prevent cutoff during perspective correction
            padding = self.padding
            height, width = image.shape[:2]
            ...
            if image_float.ndim == 2:
                # Grayscale image
                corrected = post.correct_perspective_image(image_float, pers_coef, order=2, mode='constant')
            else:
                # Multi-channel image
                corrected = np.zeros_like(image_float)
                for c in range(image_float.shape[2]):
                    corrected[:, :, c] = post.correct_perspective_image(image_float[:, :, c], pers_coef, order=2, mode='constant')
    ...
    def process_dual_images(self, left_path, right_path, output_dir=None, base_name=None):
        ...
        if self.apply_cropping:
            left_processed = self.crop_image(left_processed, 'cam0',self.crop_params)
        ...
        if self.enable_distortion_correction & self.enable_perspective_correction:
            left_processed = self.crop_image(left_processed, 'cam0',self.final_crop_params)
        
        # Process right image (cam1)
        print("\n[INFO] Processing right image (cam1)...")
        right_processed = right_image
        if self.apply_cropping:
            right_processed = self.crop_image(right_processed, 'cam1',self.crop_params)
        if self.enable_distortion_correction:
        ...
        if self.enable_distortion_correction & self.enable_perspective_correction:
            right_processed = self.crop_image(right_processed, 'cam1',self.final_crop_params)
...
def main():
    ...
    parser.add_argument("--padding", type=int, default=200, help="Padding for correction (default: 200)")
    ...
    # Only override padding if explicitly provided via command line
    if args.padding != 200:  # Default value
        processor.padding = args.padding
